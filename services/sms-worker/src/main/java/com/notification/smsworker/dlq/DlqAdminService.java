...
import com.notification.smsworker.entity.AuditLog;
import com.notification.smsworker.entity.DlqItem;
import com.notification.smsworker.idempotency.IdempotencyGuard;
import com.notification.smsworker.repository.AuditLogRepository;
import com.notification.smsworker.repository.DlqItemRepository;
...
public class DlqAdminService {
    ...
    public void retry(UUID id, String actorId) {
        ...
        kafkaTemplate.send(smsTopic, item.getNotificationId(), payload);

        item.markRetried();
        dlqItemRepository.save(item);

        audit(actorId, "DLQ_RETRY", item.getId().toString());
        log.info("DLQ item {} republished to {} by {}", item.getId(), smsTopic, actorId);
    }
